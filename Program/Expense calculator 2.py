...
def list_all_expenses():
  ...
  total_expense_desc = {}
  for values in data:
     expenditure_date = datetime.datetime.strptime(values[1], '%Y-%m-%d').date()
     if expenditure_date == datetime.date.today():
       total_expense_today += float(values[4])
     if values[3] not in total_expense_desc:
       total_expense_desc[values[3]] = float(values[4])
     else:
       total_expense_desc[values[3]] += float(values[4])
     table.insert('', END, values=values)
  return total_expense_today, total_expense_desc
def add_another_expense():
    ...
    if not date.get() or not payee.get() or not desc.get() or not amnt.get() or not MoP.get():
        ...
        connector.commit()
        clear_fields()
        total_expense_today, _ = list_all_expenses()  # update total expense
...
def remove_expense():
    ...
    if surety:
        ...
        connector.commit()
        list_all_expenses()  # update total expense
        mb.showinfo('Record deleted successfully!', 'The record you wanted to delete has been deleted successfully')
def display_total_expense():
    global total_expense_today, total_expense_desc
    total_expense_label = Label(tree_frame, text=f"Total Expense Today: {total_expense_today:.2f}", font=('Noto Sans CJK TC', 12, 'bold'))
    total_expense_label.place(side=BOTTOM, fill=X)
    for desc, amount in total_expense_desc.items():
        desc_label = Label(tree_frame, text=f"Total Expense for {desc}: {amount:.2f}", font=('Noto Sans CJK TC', 12))
        desc_label.place(x=10, y=total_expense_label.winfo_y() + 20 + (20 * total_expense_desc.index(desc)))
    # update total expense
    total_expense_today, total_expense_desc = list_all_expenses()
def view_expense_details():
  ...
  total_expense.config(text=f"Total Expense: {total_expense:.2f}")
def clear_fields():
...
def update_total_expense():
  global total_expense_today
  total_expense_today, _ = list_all_expenses()
...
root = Tk()
root.title('Group-15 Expense Calculator')
...
date = DateEntry(data_entry_frame, date=datetime.datetime.now().date(), font=entry_font, command=update_total_expense)
# Data Entry Frame
...
Button(buttons_frame, text='Display Total Expense', font=btn_font, width=25, bg=hlb_btn_bg, command=display_total_expense).place(x=335, y=130)
# Treeview Frame
...
total_expense_today, total_expense_desc = list_all_expenses()
#Total expense
total_expense= Label(tree_frame, text=f"Total Expense: {total_expense_today:.2f}", font=('Noto Sans CJK TC', 12, 'bold'))
total_expense.pack(side=BOTTOM, fill=X)
